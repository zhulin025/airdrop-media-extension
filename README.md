...
AirDrop Media 是一个 macOS 上的 Chrome/Edge 浏览器扩展。它可以让你在网页上右键图片、视频、音频或直接媒体链接，先用浏览器正常下载文件，然后自动打开 macOS AirDrop 隔空投送面板。它也支持在网页中拖拽框选截图区域，并直接通过 AirDrop 发送。
...
- 点击扩展图标或右键页面，可以拖拽框选截图区域并 AirDrop
...
扩展会在当前网页上显示一个截图框选层。拖拽选择要截图的区域后，扩展会裁剪该区域，保存到默认下载目录里的 `AirDrop-Media/` 文件夹，然后自动打开 AirDrop 面板。按 `Esc` 可以取消截图。

## 限制

- AirDrop 面板打开后，仍然需要你手动选择接收设备。
- 截图功能只能框选当前浏览器标签页内的可见网页区域，不是整页长截图，也不能截取浏览器外的桌面或其他 App。
